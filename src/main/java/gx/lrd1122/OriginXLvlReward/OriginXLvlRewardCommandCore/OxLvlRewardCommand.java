        ...
        if(sender instanceof Player){
            ...
            if(args.length > 0){
                ...
                if(args[0].equalsIgnoreCase("reload") && player.hasPermission("oxlr.reload")){
                    player.sendMessage(PlaceholderAPI.setPlaceholders(player, "%OriginXLvlReward_lrd1122_Example1%"));
                    OriginXLvlReward.load();
                    OxLoggerManager.sendMessage(player, OriginXYamlAPI.getLangString("Commands.Reload"));
                    return true;
                }
            }
        }
